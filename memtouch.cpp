...
class WorkerThread {
 public:
  WorkerThread(unsigned id_, unsigned mem_size_mb_, unsigned rw_ratio_,
               bool random_)
      : id(id_),
...
int main(int argc, char** argv) {
  ...
  for (unsigned num_thread = 0; num_thread < num_threads; num_thread++) {
    worker_storage.emplace_back(num_thread, thread_mem, rw_ratio,
                                random_access);
    thread_storage.emplace_back(std::move(
        make_unique<thread>(&WorkerThread::run, &worker_storage.back())));
  }
